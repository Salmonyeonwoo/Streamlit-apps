...
def synthesize_and_play_audio(current_lang_key):
    ...
    (TTS 안정화를 위해 LANG 딕셔너리의 텍스트를 Python에서 미리 렌더링하여 주입합니다.)
    """
    
    # 템플릿 리터럴 내부에서 L 딕셔너리를 직접 참조할 수 없으므로, 필요한 텍스트를 미리 로드
    ko_ready = "음성으로 듣기 준비됨"
    en_ready = "Ready to listen"
    ja_ready = "音声再生の準備ができました"
    
    # ⭐ 현재 언어의 텍스트를 미리 파이썬에서 가져와서 JS 문자열에 삽입
    L = LANG[current_lang_key]
    tts_generating = L.get("tts_status_generating", "오디오 생성 중...")
    tts_success = L.get("tts_status_success", "✅ 오디오 재생 완료!")
    tts_error = L.get("tts_status_error", "❌ TTS 오류 발생")
    ...
    window.speakText = function(text, langKey) {{
        ...
        // 이벤트 핸들러 설정: 미리 로드된 텍스트 사용
        utterance.onstart = () => {{
            statusElement.innerText = '{tts_generating}'; // ⭐ 수정됨
            statusElement.style.backgroundColor = '#fff3e0';
        }};
        
        utterance.onend = () => {{
            statusElement.innerText = '{tts_success}'; // ⭐ 수정됨
        ...
        utterance.onerror = (event) => {{
            statusElement.innerText = '{tts_error}'; // ⭐ 수정됨
...
LANG = {
    "ko": {
        ...
        # ⭐ 대화형/종료 메시지 (플레이스홀더 추가)
        "button_mic_input": "🎙️ 음성 입력",
        "agent_response_placeholder": "또는 텍스트로 응답하세요 (재반박 대응)", # ⭐ 추가
    ...
    "en": {
        ...
        "agent_response_placeholder": "Or respond with text (rebuttal response)", # ⭐ 추가
    ...
    "ja": {
        ...
        "agent_response_placeholder": "またはテキストで応答 (再反論の対応)", # ⭐ 추가
        ...
        if st.session_state.initial_advice_provided and not st.session_state.is_chat_ended:
            ...
            if last_role in ["customer_rebuttal", "customer_end", "supervisor"]:
                ...
                if audio_response is not None:
                    # 💡 실제 STT API 연동 로직이 필요한 부분입니다. 현재는 텍스트 플레이스홀더로 대체합니다.
                    if API_KEY:
                        try:
                            # 이 부분은 실제 STT API 호출 코드로 대체해야 합니다.
                            with st.spinner("음성 분석 및 텍스트 변환 중... (STT 기능 구현 필요)"):
                                time.sleep(1) # 가상의 변환 시간
                                # 임시 STT 플레이스홀더 (언어 감안)
                                stt_placeholder = {
                            ...
                            stt_error_message = f"STT 변환 오류: {e}"
                            st.warning(stt_error_message)
                    else:
                        st.warning("⚠️ API Key가 없거나 STT 모듈이 없어 음성 입력이 텍스트로 변환되지 않습니다.")
                        
                # 텍스트 입력이 있는 경우 처리 (텍스트 입력이 오디오보다 우선)
                if agent_response_text:
                    response_to_process = agent_response_text
                
                # 최종 응답 처리
                if response_to_process and st.session_state.agent_text_input_field != L["agent_response_placeholder"] and (stt_error_message is None):
                    st.session_state.simulator_messages.append({"role": "agent_response", "content": response_to_process})
                    st.session_state.simulator_memory.chat_memory.add_user_message(response_to_process) # 에이전트 응답을 메모리에 추가
                    
                    # 입력 필드 초기화 (다음 턴을 위해)
                    st.session_state.agent_text_input_field = ""
                    st.session_state.agent_audio_input_mic = None 
                    st.rerun()


            # --- 고객의 다음 반응 요청 버튼 및 응대 종료 버튼 ---
            # 응대 입력 필드가 사라지면 이 버튼들이 노출되어야 합니다.
            # 이 로직은 agent_response 처리가 끝난 후, 즉, 위의 if response_to_process가 실행되어 rerun 되기 전에 존재했습니다.
            # 하지만 Streamlit의 특성상 위에서 rerun이 일어나므로, 이 버튼들은 다음에 렌더링 됩니다.
            # 현재는 last_role in [...] 안에 있으므로, 에이전트 응답을 입력한 후 바로 다음 턴에서 보이지 않습니다.
            # 따라서 agent_response를 입력하기 전, 즉 last_role이 customer_rebuttal/customer_end/supervisor 일 때만 보이도록 수정합니다.
            
            if last_role not in ["agent_response"]: 
...
Requested Format: {display_type_text}"""
    ...
    st.error(L["lstm_disabled_error"])
